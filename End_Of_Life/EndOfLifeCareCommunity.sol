...
contract EndOfLifeCareCommunity {
    ...
    // =========== 服务管理与预订 ===========
    function createService(string calldata _title, uint256 _price) public {
    require(workerProfiles[msg.sender].isWorker, unicode"需先注册为护工");
    require(_price > 0, unicode"价格必须大于0");
    ...
    emit ServiceCreated(nextServiceId, _title, msg.sender, _price);
    nextServiceId++;
}

    function bookService(uint256 serviceId) public payable {
        Service memory s = services[serviceId];
        require(s.active, unicode"服务未激活或不存在");
        require(msg.value == s.price, unicode"支付金额不正确");
    ...
    function acceptOrder(uint256 orderId) public {
        ...
        require(msg.sender == o.provider, unicode"只有服务提供者可接单");
        require(o.status == OrderStatus.Created, unicode"状态不正确");
    ...
    function completeOrder(uint256 orderId) public {
        Order storage o = orders[orderId];
        require(msg.sender == o.user || msg.sender == o.provider, unicode"无权操作");
        require(o.status == OrderStatus.Accepted, unicode"仅可完成已接单订单");
        o.status = OrderStatus.Completed;
        (bool success, ) = payable(o.provider).call{value: o.amount}("");
    ...
    function cancelOrder(uint256 orderId) public {
        Order storage o = orders[orderId];
        require(msg.sender == o.user, unicode"仅用户可取消");
        require(o.status == OrderStatus.Created, unicode"仅可取消未被接单的订单");
    ...
    function rateOrder(uint256 orderId, uint8 score, string memory comment) public {
        Order storage o = orders[orderId];
        require(o.status == OrderStatus.Completed, unicode"仅可评价已完成的订单");
        require(msg.sender == o.user, unicode"仅下单用户可评价");
        require(!o.rated, unicode"已经评价过了");
        require(score >= 1 && score <= 5, unicode"评分区间1-5");
